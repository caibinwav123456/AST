...
static int cat_pred_handler(cmd_param_st* param)
{
	...
	for(int i=1;i<(int)args.size();i++)
	{
		...
		else
		{
			...
			{
				t_cat_clean_priv(newcat);
				return_msg(ERR_FILE_IO,"\'%s\': can not open file for reading\n",ph.path.c_str());
			}
...
static int write_pred_handler(cmd_param_st* param)
{
	...
	for(int i=1;i<(int)args.size();i++)
	{
		...
		else
		{
			...
			{
				t_write_clean_priv(newwrite);
				return_msg(ERR_FILE_IO,"\'%s\': can not open file for writing\n",arg.first.c_str());
			}
		}
	}
	if(!file_arg)
...
static int setlen_handler(cmd_param_st* param)
{
	...
	if(!VALID(hfile))
		return_t_msg(ERR_GENERIC,"\'%s\': open file failed\n",fullpath.c_str());
